...
const int M=1e5,N=1e5,INF=2e9;
const int mod =1e9+7;

// modulo of exponentential or for modulo inverse
ll powe(ll x, ll y) {
    x = x % mod, y = y % (mod - 1);
    ll ans = 1;
    while (y > 0) {
...
// where if the actual string is ABBA
// we make it #A#B#B#A
// so that we dont have to handle even palindromes separately
